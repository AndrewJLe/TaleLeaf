...
export const AllNotesSection: React.FC<AllNotesSectionProps> = ({
  ...
  const cardConfigBySource: Record<AllNotesSource, EntityCardConfig> = {
    character: { entityType: 'character', icon: UsersIcon, iconColor: 'amber', gradientFrom: 'emerald', nameEditMode: 'pencil', placeholder: 'Describe this character, their personality, role, background...' },
    chapter: { entityType: 'chapter', icon: BookOpenIcon, iconColor: 'blue', gradientFrom: 'blue', nameEditMode: 'pencil', placeholder: 'Chapter summary and notes...' },
    location: { entityType: 'location', icon: MapPinIcon, iconColor: 'purple', gradientFrom: 'purple', nameEditMode: 'pencil', placeholder: 'Describe this location, its significance, atmosphere...' },
    note: { entityType: 'note', icon: NotebookIcon, iconColor: 'orange', gradientFrom: 'orange', nameEditMode: 'pencil', placeholder: 'Write your note content here...' }
  };

  // Build aggregated list (stable grouped ordering)
  const aggregated: AggregatedItem[] = useMemo(() => {
    const asAgg = (src: AllNotesSource, id: string, name: string, body: string, tags: string[], position: number): AggregatedItem => ({
      key: `${src}:${id}`,
      source: src,
      id,
      name,
    ...
    const chars = (characters.items || []).slice().sort((a, b) => (a.position ?? 0) - (b.position ?? 0)).map(c => asAgg('character', c.id, c.name, c.notes, c.tags || [], c.position ?? 0));
    const chaps = (chapters.items || []).slice().sort((a, b) => (a.position ?? 0) - (b.position ?? 0)).map(ch => asAgg('chapter', ch.id, ch.name || ch.title || 'Untitled', ch.notes, ch.tags || [], ch.position ?? 0));
    const locs = (locations.items || []).slice().sort((a, b) => (a.position ?? 0) - (b.position ?? 0)).map(l => asAgg('location', l.id, l.name, l.notes, l.tags || [], l.position ?? 0));
    const nts = (notes.items || []).slice().sort((a, b) => (a.position ?? 0) - (b.position ?? 0)).map(n => asAgg('note', n.id, n.title || `Note`, n.body, n.tags || [], n.position ?? 0));
  ...
  useEffect(() => { onUnsavedChangesUpdate?.(dirtyCount > 0, dirtyCount); }, [dirtyCount, onUnsavedChangesUpdate]);

  // Parent triggers
  useEffect(() => {
    if (onSaveAllRef) onSaveAllRef.current = async () => { await Promise.all(Object.keys(dirty).filter(k => dirty[k]).map(k => handleSaveByKey(k))); };
    if (onDiscardAllRef) onDiscardAllRef.current = () => {
      setDrafts(prev => {
        const next = { ...prev };
        Object.keys(dirty).forEach(k => { delete next[k]; });
        return next;
      });
      setDirty({});
    };
  ...
  const getDisplayValue = (item: AggregatedItem) => drafts[item.key] ?? item.notes ?? '';

  const updateImmediate = async (item: AggregatedItem, patch: Partial<{ name: string; notes: string; tags: string[] }>) => {
    if (item.source === 'character') {
      await characters.update(item.id, { name: patch.name, notes: patch.notes, tags: patch.tags } as Partial<Character>);
    } else if (item.source === 'chapter') {
      await chapters.update(item.id, { name: patch.name, title: patch.name, notes: patch.notes, tags: patch.tags } as Partial<Chapter>);
    } else if (item.source === 'location') {
      await locations.update(item.id, { name: patch.name, notes: patch.notes, tags: patch.tags } as Partial<Location>);
    } else {
      await notes.update(item.id, { title: patch.name, body: patch.notes, tags: patch.tags } as Partial<BookNote>);
    }
  };
  ...
  const handleSave = async (item: AggregatedItem) => { await handleSaveByKey(item.key); };
  const handleSaveByKey = async (key: string) => {
    ...
    try {
      ...
      setTimeout(() => setShowSaved(prev => ({ ...prev, [key]: false })), 2000);
    } finally {
  ...
  const handleCancel = (item: AggregatedItem) => {
    setDrafts(prev => { const next = { ...prev }; delete next[item.key]; return next; });
    setDirty(prev => ({ ...prev, [item.key]: false }));
  };

  // Tags add/remove and name changes via onUpdateEntity should persist immediately
  const handleUpdateEntity = async (_index: number, entity: { id: string; name: string; notes: string; tags: string[] }) => {
    const item = aggregated.find(a => a.id === entity.id && a.name !== undefined && a.key.startsWith('character:') ? true : true);
    // Determine source reliably: look up across sources in order of likelihood
    let src: AggregatedItem | undefined = aggregated.find(a => a.id === entity.id && entity.name === a.name && entity.notes === a.notes);
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    const sourceItems = aggregated.filter(a => a.source === item.source).sort((a, b) => a.position - b.position);
    const currentIdx = sourceItems.findIndex(a => a.id === item.id);
    const neighborIdxInSource = sourceItems.findIndex(a => a.id === neighbor.id);
    if (currentIdx < 0 || neighborIdxInSource < 0) return;
    [sourceItems[currentIdx], sourceItems[neighborIdxInSource]] = [sourceItems[neighborIdxInSource], sourceItems[currentIdx]];
    const orderedIds = sourceItems.map(s => s.id);
    if (item.source === 'character' && characters.reorder) await characters.reorder(orderedIds);
    if (item.source === 'chapter' && chapters.reorder) await chapters.reorder(orderedIds);
    if (item.source === 'location' && locations.reorder) await locations.reorder(orderedIds);
    if (item.source === 'note' && notes.reorder) await notes.reorder(orderedIds);
  };
  ...
  const changeNameDraft = (k: string, v: string) => setNameDrafts(prev => ({ ...prev, [k]: v }));
  const finishNameEdit = async (k: string, save: boolean) => {
    ...
    setNameDrafts(prev => { const next = { ...prev }; delete next[k]; return next; });
  };
      ...
      <div className="p-4 sm:p-6 bg-gray-50 rounded-xl border border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-orange-100 rounded-lg flex-shrink-0"><NotebookIcon size={20} className="text-orange-600" /></div>
            <div>
              <h4 className="text-lg font-semibold text-gray-900">All Notes ({filtered.length}{tokens.length > 0 ? ` / ${aggregated.length}` : ''})</h4>
      ...
      <div className="space-y-4">
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-lg font-medium text-gray-700">No notes match your filter</p>
            <p className="text-sm text-gray-500">Try a different tag or clear the search.</p>
          </div>
        )}
      </div>
    </div>
