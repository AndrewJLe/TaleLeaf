...
export function BaseEntityCard<T extends BaseEntity>({
  ...
  const [tagColorOverrides, setTagColorOverrides] =
    useState<Record<string, string>>(tagColorMap);
  ...
  const GRADIENT_CLASS_MAP: Record<string, string> = {
    emerald:
      "from-emerald-100 to-white border-emerald-100 hover:border-emerald-400",
    green: "from-green-100 to-white border-green-100 hover:border-green-400",
    purple:
      "from-purple-100 to-white border-purple-100 hover:border-purple-400",
    orange:
      "from-orange-100 to-white border-orange-100 hover:border-orange-400",
    ...
    indigo:
      "from-indigo-100 to-white border-indigo-100 hover:border-indigo-400",
  };

  const gradientClass =
    GRADIENT_CLASS_MAP[gradientFrom] || GRADIENT_CLASS_MAP["emerald"];
  ...
  const handleCreateTag = async () => {
    ...
    const color =
      newTagColorDraft ||
      colorForTagName(raw, { ...tagColorMap, ...tagColorOverrides });
  ...
  const renderNameField = () => {
      ...
      return (
        <div className="flex items-center gap-2 w-full">
          <div className="font-semibold text-gray-900 text-lg truncate">
            {entity.name || `${config.entityType} name`}
          </div>
          <Tooltip
            text={`Edit ${config.entityType} name`}
            id={`edit-${config.entityType}-name-${entity.id}`}
          >
            <button
              type="button"
              onClick={(e) => {
    ...
    return (
      <input
        ...
        value={entity.name}
        onChange={(e) =>
          onUpdateEntity(index, { ...entity, name: e.target.value } as T)
        }
  ...
  return (
    <div
      className={`group relative overflow-hidden p-4 sm:p-6 bg-gradient-to-br ${gradientClass} hover:border-2 rounded-xl shadow-sm hover:shadow-md transition-all duration-200`}
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div
            ...
            tabIndex={0}
            onClick={() => {
              /* TODO: open entity details modal */
            }}
          >
            ...
            <div className="flex flex-wrap items-center gap-2">
              {(entity.tags || []).map((tag) => {
                ...
                const tokens = (highlightTokens || [])
                  ...
                  .filter(Boolean);
                const isHighlighted =
                  tokens.length > 0 && tokens.some((tok) => lc.includes(tok));
                return (
                  <div
                    key={tag}
                    ...
                    style={{
                      ...
                      boxShadow: isHighlighted
                        ? `0 0 0 2px ${baseColor}, 0 0 6px 2px ${hexToRgba(baseColor, 0.35)}`
                        : undefined,
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2 order-2 sm:order-1">
            <button
              ...
              className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-all shadow-sm flex items-center gap-2 ${
                isSaving
        ...
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white border border-red-200 rounded-xl shadow-lg p-5 w-full max-w-sm">
            <h4 className="text-sm font-semibold text-gray-900 mb-2">
              Delete {config.entityType}?
            </h4>
            <p className="text-xs text-gray-600 mb-4">
              This will remove this {config.entityType} from active view.
              (Soft-deleted)
            </p>
