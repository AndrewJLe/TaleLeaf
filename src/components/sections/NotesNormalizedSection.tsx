...
import { EyeIcon, EyeOffIcon, NotebookIcon, PlusIcon, SaveIcon, UndoIcon } from '../ui/Icons';
...
interface NotesNormalizedSectionProps {
  ...
  addNote: (note: Omit<BookNote, 'id' | 'createdAt' | 'updatedAt' | 'bookId'>) => Promise<void> | void;
  immediateUpdateNote: (id: string, updates: Partial<BookNote>) => Promise<void> | void;
...
interface NoteEntityProxy { id: string; name: string; notes: string; tags: string[]; }
...
export const NotesNormalizedSection: React.FC<NotesNormalizedSectionProps> = ({
  ...
  useEffect(() => { onUnsavedChangesUpdate?.(hasUnsavedChanges, dirtyNoteIds.length); }, [hasUnsavedChanges, dirtyNoteIds, onUnsavedChangesUpdate]);
  ...
  const handleAddNote = async () => {
    ...
    await addNote({
      ...
      body: '',
      tags: [],
      position: (count === 0 ? 0 : Math.max(...notes.map(n => n.position)) + 1000),
  ...
  const proxyEntities: NoteEntityProxy[] = useMemo(() => notes
    .slice()
    .sort((a, b) => a.position - b.position)
    .map(n => ({ id: n.id, name: n.title || '', notes: n.body, tags: n.tags })), [notes]);
  ...
  const handleCancel = (entity: NoteEntityProxy) => { cancelNote(entity.id); };
  const handleUpdateEntity = (_index: number, entity: NoteEntityProxy) => {
    ...
    updateDraft(entity.id, { title: entity.name, body: entity.notes, tags: entity.tags });
  };
  ...
  const finishNameEdit = async (id: string, save: boolean) => {
    ...
    setNameDrafts(prev => { const next = { ...prev }; delete next[id]; return next; });
  };
  ...
  const spoilerAction = (noteId: string) => {
    ...
    if (!note) return null;
    return (
      <Tooltip text={note.spoilerProtected ? 'Disable spoiler protection' : 'Enable spoiler protection'} id={`spoiler-${noteId}`}>
        <Button onClick={() => toggleSpoiler(noteId)} variant="ghost" size="sm" className={note.spoilerProtected ? 'text-yellow-600' : 'text-gray-400'}>
          {note.spoilerProtected ? <EyeOffIcon size={14} /> : <EyeIcon size={14} />}
        </Button>
      </Tooltip>
    );
  };
      ...
      <div className="p-4 sm:p-6 bg-gray-50 rounded-xl border border-gray-200 flex flex-col gap-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-orange-100 rounded-lg"><NotebookIcon size={20} className="text-orange-600" /></div>
            <h4 className="text-lg font-semibold text-gray-900">Notes ({notes.length})</h4>
            {hasUnsavedChanges && (
              <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full">{dirtyNoteIds.length} unsaved</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            {hasUnsavedChanges && (
              <Button onClick={saveAllNotes} size="sm" variant="primary" disabled={isLoading} className="bg-emerald-600 text-white hover:bg-emerald-700">
                <SaveIcon size={14} /><span className="hidden sm:inline ml-1">Save All</span>
              </Button>
            )}
            {hasUnsavedChanges && (
              <Button onClick={discardAllChanges} size="sm" variant="secondary" className="bg-gray-600 text-white hover:bg-gray-700">
                <UndoIcon size={14} /><span className="hidden sm:inline ml-1">Discard All</span>
              </Button>
            )}
            <div className="flex items-stretch gap-2">
              ...
              />
              <Button onClick={handleAddNote} variant="primary" disabled={isLoading}>
      ...
      <div className="space-y-4">
        ...
        {proxyEntities.length === 0 && !isLoading && (
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm text-gray-500">Add your first note to get started.</p>
          </div>
        )}
        {isLoading && (
