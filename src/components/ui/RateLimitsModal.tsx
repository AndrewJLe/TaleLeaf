        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
            ...
            <div className="text-xs text-amber-800 space-y-1">
              <div>
                • <strong>Rolling Window:</strong> OpenAI tracks your usage over
                a sliding 60-second window
              </div>
              <div>
                • <strong>TPM (Tokens Per Minute):</strong> Maximum tokens you
                can use in any 60-second period
              </div>
              <div>
                • <strong>Long Cooldowns:</strong> Heavy usage can result in
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <div className="text-red-700">
                      <strong>Provider:</strong> {rateLimitInfo.provider}
                    </div>
                    <div className="text-red-700">
                      <strong>Used:</strong>{" "}
                      {formatTokens(rateLimitInfo.tokensUsed)} tokens
                    </div>
                    <div className="text-red-700">
                  ...
                  <div>
                    <div className="text-red-700">
                      <strong>Limit:</strong>{" "}
                      {formatTokens(rateLimitInfo.tokensPerMinute)} TPM
                    </div>
                    <div className="text-red-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {AI_PROVIDERS.map((provider) => {
              ...
              const isRateLimited =
                rateLimitInfo?.provider === provider.id &&
                rateLimitInfo.retryAfter &&
                rateLimitInfo.retryAfter > 0;
                ...
                >
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex justify-between items-center mb-1">
                          <span className="text-sm text-gray-600">
                            Usage{" "}
                            {status.isSimulated ? "(Estimated)" : "(API Data)"}
                            {!status.isSimulated && (
                              <span className="ml-1 text-amber-700">📡</span>
                            )}
                          </span>
                          <span className="text-sm font-mono">
                            {formatTokens(status.used)} /{" "}
                            {formatTokens(status.limit)}
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (status.used / status.limit) * 100)}%`,
                            }}
                          />
                        </div>
                        <div className="flex justify-between text-xs text-gray-500 mt-1">
                          <span>
                            Available: {formatTokens(status.available)}
                          </span>
                          <span>
                      ...
                      <div className="text-xs text-gray-500 space-y-1">
                        <div>
                          ...
                          {formatTokens(status.limit)} tokens/minute
                        </div>
                        <div>
                          <strong>Tier:</strong> {provider.tier}
                        </div>
                        <div>
                          <strong>Cost:</strong> {provider.costEstimate}
                        </div>
                        {status.isSimulated && (
                          <div className="text-amber-600">
                            ⚠️ Estimated until you make a request
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {!status && (
                    <div className="text-xs text-gray-500">
                      No usage data available. Status will appear after making
                      requests.
                    </div>
                  )}
                </div>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            ...
            <div className="text-xs text-gray-700 space-y-1">
              <div>
                • <strong>Use GPT-4o-mini:</strong> Higher rate limits (100K TPM
                vs 30K TPM for GPT-4o)
              </div>
              <div>
                • <strong>Smaller contexts:</strong> Use the context window to
                limit input size
              </div>
              <div>
                • <strong>Batch operations:</strong> Wait between multiple AI
                generations
              </div>
              <div>
                • <strong>Monitor usage:</strong> Check this panel before large
                operations
              </div>
              <div>
                • <strong>Upgrade tier:</strong> Higher OpenAI tiers have higher
                rate limits
              </div>
            </div>
          </div>
