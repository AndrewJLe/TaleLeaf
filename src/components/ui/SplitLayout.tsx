...
export const SplitLayout: React.FC<SplitLayoutProps> = ({
  ...
  return (
    ...
    >
      {/* Left Panel */}
      <div
        className={`flex flex-col bg-white transition-all duration-300 ease-in-out min-w-0 ${
          isLeftCollapsed ? "overflow-hidden" : "overflow-hidden"
        }`}
      >
        {/* Left Panel Header */}
        ...
        {/* Left Panel Content */}
        <div
          className={`flex-1 overflow-y-auto overflow-x-hidden ${isLeftCollapsed ? "hidden" : "block"}`}
        >
          {leftPanel}
        </div>
      </div>
      ...
      {/* Right Panel */}
      <div
        className={`flex flex-col bg-gray-50 transition-all duration-300 ease-in-out min-w-0 ${
          isRightCollapsed ? "overflow-hidden" : "overflow-hidden"
        }`}
      >
        {/* Right Panel Header */}
        <div className="flex items-center justify-between p-2 sm:p-4 border-b border-gray-200 bg-white flex-shrink-0">
        ...
        {/* Right Panel Content */}
        <div
          className={`flex-1 overflow-y-auto overflow-x-hidden ${isRightCollapsed ? "hidden" : "block"}`}
        >
          {rightPanel}
        </div>
      </div>
