...
export default function UploadForm({ onAdd }: Props) {
  ...
  const [processedUpload, setProcessedUpload] = useState<BookUpload | null>(
    null,
  );
  const [loading, setLoading] = useState(false);
  const [loadingMessage, setLoadingMessage] = useState<string>("");
  const [coverDataUrl, setCoverDataUrl] = useState<string | null>(null);
  const [manualPageCount, setManualPageCount] = useState<number | undefined>(
    undefined,
  );
        ...
        try {
          pageTexts = await PDFUtils.extractAllPageTexts(file, {
            ...
            onProgress: (current, total) => {
              if (current % 25 === 0) {
                setLoadingMessage(
                  `Extracting text: page ${current}/${total}...`,
                );
              }
            },
          });
          ...
          console.warn(
            "PDF text extraction failed, continuing without page texts.",
            textErr,
          );
        }
  ...
  async function handleAdd() {
    ...
    const book = {
      ...
      pdfPath,
      pdfPageCount:
        processedUpload?.type === "pdf" ? processedUpload.pageCount : undefined,
    };
            ...
            <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
              ...
              <div className="text-sm text-emerald-600">
                ...
                <span className="capitalize">
                  {processedUpload.type} document
                </span>
                {processedUpload.pageCount === 1 &&
                  processedUpload.type === "pdf" && (
          ...
          {showManualPageInput && processedUpload?.type === "pdf" && (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              <p className="text-xs text-amber-600 mt-1">
                💡 If the auto-detected page count (1 page) seems wrong for your
                PDF, enter the correct number here.
              </p>
            </div>
          )}
        ...
        <div className="flex items-center gap-3">
          ...
          {processedUpload && !loading && (
            <span className="text-sm text-emerald-700">
              ✅ Ready to add: {manualPageCount || processedUpload.pageCount}{" "}
              pages from {processedUpload.filename}
            </span>
          )}
          {pastedText && !processedUpload && !loading && (
            <span className="text-sm text-emerald-700">
              ✅ Ready to add: {splitIntoPages(pastedText.trim()).length} pages
              from pasted text
            </span>
          )}
        </div>
