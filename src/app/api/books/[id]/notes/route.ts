...
export async function GET(
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("book_notes")
      .select(
        "id,book_id,title,body,position,spoiler_protected,min_visible_page,group_id,created_at,updated_at,deleted_at",
      )
      .eq("book_id", bookId)
      .is("deleted_at", null)
...
export async function POST(
  ...
  try {
    ...
    const { data: inserted, error: insErr } = await supabase
      ...
      .insert(insertBody)
      .select(
        "id,book_id,title,body,position,spoiler_protected,min_visible_page,group_id,created_at,updated_at",
      )
      .single();
      ...
      if (tagNames.length) {
        ...
        for (const name of tagNames) {
          try {
            const { error: tagErr } = await supabase
              .from("book_tags")
              .upsert({ book_id: bookId, name })
              .select("id")
              .single();
            if (tagErr && tagErr.code !== "23505")
              console.warn("tag upsert issue", tagErr);
          } catch {
            /* ignore */
          }
        }
        ...
        for (const name of tagNames) {
          ...
          try {
            await supabase
              .from("book_note_tags")
              .upsert({ note_id: inserted.id, tag_id: tagId });
          } catch {
            /* ignore */
          }
        }
    ...
    const { data: insertedJoins } = await supabase
      .from("book_note_tags")
      .select("tag:book_tags(name)")
      .eq("note_id", inserted.id);
    const insertedTagNames = (insertedJoins || [])
      .map((j: any) => j.tag?.name)
      .filter((n: string) => !!n);
...
export async function PUT(
  ...
  try {
    ...
    if (payload.spoilerProtected !== undefined)
      updateBody.spoiler_protected = payload.spoilerProtected;
    if (payload.minVisiblePage !== undefined)
      updateBody.min_visible_page = payload.minVisiblePage;
    ...
    if (payload.tags !== undefined) {
      ...
      const existingNames = (existingJoins || [])
        .map((j: any) => j.tag?.name)
        .filter((n: string) => !!n);
      ...
      if (toAdd.length || toRemove.length) {
        ...
        const { data: involvedTags } = await supabase
          .from("book_tags")
          .select("id,name")
          .eq("book_id", bookId)
          .in("name", involved);
        const idByName: Record<string, string> = {};
        (involvedTags || []).forEach((r) => {
        ...
        for (const name of toAdd) {
          ...
          if (tagId) {
            try {
              await supabase
                .from("book_note_tags")
                .upsert({ note_id: payload.id, tag_id: tagId });
            } catch {
              /* ignore */
            }
          }
    ...
    const { data: updatedJoins } = await supabase
      .from("book_note_tags")
      .select("tag:book_tags(name)")
      .eq("note_id", updated.id);
    const updatedTagNames = (updatedJoins || [])
      .map((j: any) => j.tag?.name)
      .filter((n: string) => !!n);
