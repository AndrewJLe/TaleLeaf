...
import {
  buildContextWindowResult,
  buildPageFocusedContextWindowResult,
  extractExplicitPageFromQuestion,
} from "../../../../../lib/server/context-window";
...
export async function POST(
  ...
  try {
    ...
    try {
      console.log("[debug] context-window incoming", {
        bookId,
        selection,
        question,
      });
    ...
    try {
      console.log("context-window request", {
        bookId,
        userId: user.id,
        selection,
        question,
    ...
    if (error?.message === "context-window-data-missing") {
      return NextResponse.json(
        { ready: false, reason: "context-window-data-missing" },
        { status: 202 },
      );
    }
