...
export async function GET(
  ...
  try {
    ...
    const { data: locs, error: lErr } = await supabase
      .from("book_locations")
      .select(
        "id,name,notes,parent_id,position,depth,created_at,updated_at,deleted_at",
      )
      .eq("book_id", bookId)
      .is("deleted_at", null)
    ...
    let joinsRes: any = await supabase
      .from("book_location_tags")
      .select("location_id, tag_id, color_override, book_id, created_at")
      .eq("book_id", bookId);
    if (joinsRes.error && /created_at/.test(joinsRes.error.message || "")) {
      joinsRes = await supabase
        .from("book_location_tags")
        .select("location_id, tag_id, color_override, book_id")
        .eq("book_id", bookId);
    }
    ...
    (joins || []).forEach((j: any) => {
      ...
      if (!tag) return;
      if (!byLoc[j.location_id])
        byLoc[j.location_id] = { tags: [], tagColors: {} };
...
interface UpsertLocationPayload {
  id?: string;
  name: string;
  notes?: string;
...
async function upsertTagsAndJoins(
  ...
  bookId: string,
  locationId: string,
  tags: UpsertLocationPayload["tags"],
) {
...
export async function POST(
  ...
  try {
    ...
    const { data: inserted, error: insErr } = await supabase
      ...
      .select("id,name,notes,parent_id,position,depth,created_at,updated_at")
      .single();
