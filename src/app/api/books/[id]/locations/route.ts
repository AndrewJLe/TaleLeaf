...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data: locs, error: lErr } = await supabase.from('book_locations').select('id,name,notes,parent_id,position,depth,created_at,updated_at').eq('book_id', bookId).order('position', { ascending: true });
    if (lErr) throw lErr;
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { error: delErr } = await supabase.from('book_locations').delete().eq('id', locationId);
    if (delErr) throw delErr;
