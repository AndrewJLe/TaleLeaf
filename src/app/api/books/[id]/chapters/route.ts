...
export async function GET(
  ...
  try {
    ...
    let joinsRes: any = await supabase
      .from("book_chapter_tags")
      .select("chapter_id, tag_id, color_override, book_id, created_at")
      .eq("book_id", bookId);
    if (joinsRes.error && /created_at/.test(joinsRes.error.message || "")) {
      joinsRes = await supabase
        .from("book_chapter_tags")
        .select("chapter_id, tag_id, color_override, book_id")
        .eq("book_id", bookId);
    }
    ...
    (joins || []).forEach((j: any) => {
      ...
      if (!tag) return;
      if (!byChapter[j.chapter_id])
        byChapter[j.chapter_id] = { tags: [], tagColors: {} };
      byChapter[j.chapter_id].tags.push(tag.name);
      byChapter[j.chapter_id].tagColors[tag.name] =
...
interface UpsertChapterPayload {
  id?: string;
  title: string;
  position?: number;
  content?: string;
...
async function upsertTagsAndJoins(
  ...
  bookId: string,
  chapterId: string,
  tags: UpsertChapterPayload["tags"],
) {
...
export async function POST(
  ...
  try {
    ...
    const { data: inserted, error: insErr } = await supabase
      ...
      .insert(insertBody)
      .select(
        "id,title,notes,summary,analysis,position,number,created_at,updated_at",
      )
      .single();
    ...
    return NextResponse.json({
      chapter: {
        ...
        number: (inserted as any).number,
        notes: inserted.notes,
        content: inserted.notes,
