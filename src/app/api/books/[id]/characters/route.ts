...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    let joinsRes: any = await supabase.from('book_character_tags').select('character_id, tag_id, color_override, created_at, book_id').eq('book_id', bookId);
    if (joinsRes.error && /created_at/.test(joinsRes.error.message || '')) {
      // Fallback if older schema without created_at column
      joinsRes = await supabase.from('book_character_tags').select('character_id, tag_id, color_override, book_id').eq('book_id', bookId);
    }
    if (joinsRes.error) throw joinsRes.error;
    const joins = joinsRes.data;
    const { data: tags, error: tErr } = await supabase.from('book_tags').select('id,name,color,book_id').eq('book_id', bookId);
    if (tErr) throw tErr;
    const tagMap = Object.fromEntries((tags || []).map(t => [t.id, t]));
    const byChar: Record<string, { tags: string[]; tagColors: Record<string, string> }> = {};
    (joins as any[] || []).forEach((j: any) => {
      const tag = tagMap[j.tag_id];
      if (!tag) return;
      if (!byChar[j.character_id]) byChar[j.character_id] = { tags: [], tagColors: {} };
      byChar[j.character_id].tags.push(tag.name);
      byChar[j.character_id].tagColors[tag.name] = j.color_override || tag.color;
    });
    ...
    if (debug) {
      ...
      console.log('[characters GET debug]', { bookId, userId: user.id, rowCount: result.length, rawCharsCount: chars?.length, joins: joins?.length, tags: tags?.length });
      return NextResponse.json({ characters: result, _debug: { rawChars: chars, joins, tags } });
    }
    // Minimal log when zero rows returned to help diagnose silent filtering (e.g., RLS)
    if (result.length === 0) {
      console.log('[characters GET] no characters returned', { bookId, userId: user.id, rawCharsCount: chars?.length, charsResError: charsRes.error });
    }
    return NextResponse.json({ characters: result });
  } catch (e: any) {
    const status = e.message === 'not found' ? 404 : 500;
    console.error('[characters GET error]', { bookId, userId: user.id, error: e?.message, stack: e?.stack });
    return NextResponse.json({ error: e.message, detail: e?.hint || e?.details || null }, { status });
  }
}

interface UpsertCharacterPayload { id?: string; name: string; notes?: string; position?: number; tags?: (string | { name: string; color?: string; overrideColor?: string })[] }

async function upsertTagsAndJoins(supabase: any, bookId: string, characterId: string, tags: UpsertCharacterPayload['tags']) {
  ...
  for (const ti of normalized) {
    ...
    if (upErr) {
      ...
      if (/no unique|no\s+unique\s+or\s+exclusion/i.test(upErr.message || '')) {
        ...
        const { data: existing, error: selErr } = await supabase.from('book_tags').select('id').eq('book_id', bookId).eq('name', ti.norm).maybeSingle();
  ...
  for (const tr of tagRows || []) {
    ...
    const { error: joinErr } = await supabase.from('book_character_tags').upsert({ character_id: characterId, tag_id: tr.id, color_override: overrideColor, book_id: bookId }, { onConflict: 'character_id,tag_id' });
    if (joinErr) throw joinErr;
  }
  // Remove stale joins
  const { data: existingJoins, error: exErr } = await supabase.from('book_character_tags').select('tag_id').eq('character_id', characterId);
  if (exErr) throw exErr;
  for (const ej of existingJoins || []) {
    if (!wantedIds.has(ej.tag_id)) {
      const { error: delErr } = await supabase.from('book_character_tags').delete().eq('character_id', characterId).eq('tag_id', ej.tag_id);
  ...
  try {
    ...
    for (const tbl of tables) {
      const { data: rows, error: rErr } = await supabase.from(tbl.table).select(tbl.col).eq('book_id', bookId);
      if (!rErr) {
        (rows || []).forEach((r: any) => { if (r[tbl.col]) usedTagIds.add(r[tbl.col]); });
      }
    }
    // Delete tags not in usedTagIds
    const { data: allTags, error: allErr } = await supabase.from('book_tags').select('id').eq('book_id', bookId);
    if (!allErr) {
      const orphanIds = (allTags || []).map((t: any) => t.id).filter((id: string) => !usedTagIds.has(id));
      if (orphanIds.length) {
        const { error: delTagsErr } = await supabase.from('book_tags').delete().in('id', orphanIds).eq('book_id', bookId);
        if (delTagsErr) {
...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    await ensureBookOwnership(supabase, bookId, user.id);
    let insertedRes = await supabase.from('book_characters').insert({ book_id: bookId, name: payload.name, notes: payload.notes || '', position: payload.position }).select('id,book_id,name,notes,position,created_at,updated_at').single();
    if (insertedRes.error && /notes/.test(insertedRes.error.message || '')) {
      insertedRes = await supabase.from('book_characters').insert({ book_id: bookId, name: payload.name, position: payload.position }).select('id,book_id,name,position,created_at,updated_at').single();
    }
    if (insertedRes.error) throw insertedRes.error;
    const inserted = insertedRes.data;
    if (payload.tags && payload.tags.length) await upsertTagsAndJoins(supabase, bookId, inserted.id, payload.tags);
    return NextResponse.json({ character: { id: inserted.id, bookId: inserted.book_id, name: inserted.name, notes: inserted.notes, position: inserted.position, tags: payload.tags?.map(t => typeof t === 'string' ? normalizeTagName(t) : normalizeTagName(t.name)) || [], createdAt: inserted.created_at, updatedAt: inserted.updated_at } });
  } catch (e: any) {
    const status = e.message === 'not found' ? 404 : 500;
...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    await ensureBookOwnership(supabase, bookId, user.id);
    const { data: existing, error: exErr } = await supabase.from('book_characters').select('id').eq('id', payload.id).eq('book_id', bookId).maybeSingle();
    if (exErr) throw exErr;
    if (!existing) return NextResponse.json({ error: 'not found' }, { status: 404 });
    let updatedRes = await supabase.from('book_characters').update({ name: payload.name, notes: payload.notes || '', position: payload.position }).eq('id', payload.id).select('id,book_id,name,notes,position,created_at,updated_at').single();
    if (updatedRes.error && /notes/.test(updatedRes.error.message || '')) {
      updatedRes = await supabase.from('book_characters').update({ name: payload.name, position: payload.position }).eq('id', payload.id).select('id,book_id,name,position,created_at,updated_at').single();
    }
    ...
    return NextResponse.json({ character: { id: updated.id, bookId: updated.book_id, name: updated.name, notes: updated.notes, position: updated.position, tags: (payload.tags || []).map(t => typeof t === 'string' ? normalizeTagName(t) : normalizeTagName(t.name)), createdAt: updated.created_at, updatedAt: updated.updated_at } });
  } catch (e: any) {
    const status = e.message === 'not found' ? 404 : 500;
    return NextResponse.json({ error: e.message }, { status });
  }
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!charId) return NextResponse.json({ error: 'id required' }, { status: 400 });
  const { id: bookId } = await context.params;
  const authHeader = req.headers.get('authorization');
  const token = authHeader?.startsWith('Bearer ') ? authHeader.slice(7) : undefined;
  const supabase = createServerSupabase(token);
