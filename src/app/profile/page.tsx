...
export default function ProfilePage() {
  ...
  const { deleteBook: deleteBookFromDB, isLoading: isDeletingFromDB } =
    useBookPersistence();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabaseClient
        .from("books")
        .select(
          "id,title,cover_url,window_start,window_end,created_at,updated_at",
        )
  ...
  const unsyncedIds = new Set(
    books.filter((b) => !remoteIds.has(b.id)).map((b) => b.id),
  );
  ...
  const confirmDeleteBook = async () => {
    ...
    try {
      ...
      setBooks((prevBooks) =>
        prevBooks.filter((b) => b.id !== deleteConfirm.id),
      );
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 via-emerald-25 to-amber-50/30 p-6">
        <div className="max-w-md w-full space-y-6">
          <div className="text-center space-y-3">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-emerald-600 text-white text-3xl">
              🍃
            </div>
            ...
            <p className="text-sm text-emerald-700 leading-relaxed">
              Create or access your TaleLeaf library with a magic link. Your
              books and notes sync securely to the cloud.
            </p>
          </div>
          <AuthPanel />
          <div className="text-center text-xs text-emerald-500">
            By continuing you agree to basic usage of cookies/local storage for
            session & book data.
          </div>
        </div>
      </div>
      ...
      <div className="relative max-w-6xl mx-auto p-8">
          ...
          <div className="mb-6 p-4 rounded-lg border border-amber-300 bg-amber-50 text-amber-800 text-sm flex items-start gap-3">
            ...
            <div>
              <div className="font-medium mb-0.5">
                Cloud sync & sign-in are not configured
              </div>
              <p>
                Set{" "}
                <code className="px-1 py-0.5 bg-white/60 rounded border border-amber-200">
                ...
                in a <code>.env.local</code> file and restart the dev server to
                enable sign-in. Local-only mode is currently active.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:block">
              ...
              <p className="text-sm text-emerald-600">
                {mergedCount} book{mergedCount !== 1 ? "s" : ""} in your
                collection
              </p>
            </div>
          </div>
                      ...
                      try {
                        ...
                        const { error: bookErr } = await supabaseClient
                          ...
                          .upsert({
                            ...
                            window_end:
                              b.window?.end || Math.min(50, b.pages || 50),
                        ...
                        const sectionMap = [
                          {
                            type: "characters",
                            data: { items: b.sections?.characters || [] },
                          },
                          {
                            type: "chapters",
                            data: { items: b.sections?.chapters || [] },
                          },
                          {
                            type: "locations",
                            data: { items: b.sections?.locations || [] },
                          },
                          {
                            type: "notes",
                            data: { content: b.sections?.notes || "" },
                          },
                        ];
        ...
        {isSupabaseEnabled && session && (
          <div className="mb-4 p-3 text-xs rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-800 font-mono flex flex-wrap gap-3">
            ...
            {remoteError && (
              <div className="text-red-600">remoteError:{remoteError}</div>
            )}
            <button
              onClick={async () => {
                const { data, error } = await supabaseClient
        ...
        <div className="md:hidden mb-6 p-4 bg-white/60 backdrop-blur-sm rounded-lg border border-emerald-200">
          ...
          <p className="text-sm text-emerald-600">
            {mergedCount} book{mergedCount !== 1 ? "s" : ""} in your collection
          </p>
        </div>
          ...
          {mergedCount === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-emerald-600 mb-6 max-w-md mx-auto">
                Start your reading journey by adding your first book. Upload a
                PDF and begin tracking characters, locations, and plot details.
              </p>
              <button
