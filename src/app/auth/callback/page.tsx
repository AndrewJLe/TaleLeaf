    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 via-emerald-25 to-amber-50/30 p-6">
      <div className="max-w-sm w-full bg-white/80 backdrop-blur-sm border border-emerald-200 rounded-xl p-6 text-center space-y-4">
        <div className="flex items-center justify-center">
          <div className="w-14 h-14 rounded-2xl bg-emerald-600 flex items-center justify-center text-white text-2xl">🍃</div>
        </div>
        <h1 className="text-xl font-semibold text-emerald-900">Authenticating</h1>
        {!error ? (
          ...
          <div className="space-y-3">
            ...
            <button onClick={() => router.replace('/profile')} className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700">Back to Profile</button>
          </div>
        )}
        <p className="text-[10px] text-emerald-500">You can close this tab if it doesn't redirect shortly.</p>
      </div>
    </div>
  );
}
