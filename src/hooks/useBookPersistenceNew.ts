...
export function useBookPersistence(): UseBookPersistenceResult {
  ...
  const saveBook = useCallback(async (book: Book) => {
    ...
    try {
      ...
      if (bookError) {
        ...
        throw new Error(
          `Failed to save book metadata: ${bookError.message || "Unknown database error"}`,
        );
      }
  ...
  const saveSections = useCallback(
    async (bookId: string, sections: any, window: any) => {
      if (!isSupabaseEnabled || !supabaseClient) {
      ...
      try {
        ...
        for (const payload of sectionPayloads) {
          console.log(
            `Saving ${payload.type} section with ${JSON.stringify(payload.data).length} characters of data`,
          );
          ...
          if (sectionError) {
            console.error(
              `Failed to save ${payload.type} section:`,
              sectionError,
            );
        ...
        if (window) {
          ...
          if (windowError) {
            ...
            throw new Error(
              `Failed to update window: ${windowError.message || "Unknown database error"}`,
            );
          }
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to save sections";
        setError(errorMessage);
        throw err;
      } finally {
        setIsLoading(false);
  ...
  const deleteBook = useCallback(async (bookId: string) => {
    ...
    try {
      ...
      if (deleteError) {
        ...
        throw new Error(
          `Failed to delete book: ${deleteError.message || "Unknown database error"}`,
        );
      }
