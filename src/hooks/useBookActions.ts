...
export const useBookActions = (
  ...
  const updateSections = useCallback(
    async (sectionUpdates: Partial<Book["sections"]>) => {
  ...
  const batchUpdateCharacters = useCallback(
    async (updatedCharacters: Character[]) => {
  ...
  const batchUpdateLocations = useCallback(
    async (updatedLocations: Location[]) => {
  ...
  const generateCharacters = useCallback(async () => {
    try {
      ...
      const aiCharacters = await aiService.generateCharactersFromBook(
        book,
        book.window.start,
        book.window.end,
      );
  ...
  const enhanceCharacter = useCallback(
    async (index: number) => {
      try {
        ...
        const contextText = aiService.extractContextText(
          book,
          book.window.start,
          book.window.end,
        );
  ...
  const generateChapterSummary = useCallback(
    async (index: number) => {
      try {
        ...
        const summary = await aiService.generateChapterSummary(
          contextText,
          chapter.name,
        );
