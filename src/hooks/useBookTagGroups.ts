...
interface UseBookTagGroupsResult {
  ...
  upsertGroup: (
    group: Partial<BookNoteGroup> & { name: string; color: string },
  ) => Promise<string | void>;
...
export function useBookTagGroups(
  bookId: string,
  enabled: boolean = true,
): UseBookTagGroupsResult {
      ...
      try {
        ...
        if (!existing.data) {
          const ins = await supabaseClient
            .from("book_tags")
            .insert({ book_id: bookId, name: normalized, color })
            .select();
          if (ins.error) {
  ...
  const upsertGroup = useCallback(
    async (group: Partial<BookNoteGroup> & { name: string; color: string }) => {
      if (!enabled || !isSupabaseEnabled || !supabaseClient) return;
      const payload: any = {
        book_id: bookId,
        name: group.name,
