...
export async function uploadPDF(userId: string, bookId: string, file: File | Blob, filename: string) {
  if (!supabaseClient) throw new Error('Supabase not initialized');
  const path = `${userId}/${bookId}/${Date.now()}-${filename}`;
  const { error } = await supabaseClient.storage.from(BUCKET).upload(path, file, { upsert: false, cacheControl: '3600', contentType: 'application/pdf' });
