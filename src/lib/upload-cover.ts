...
export async function uploadCover(file: File, bookId: string): Promise<string | null> {
  ...
  try {
    ...
    if (scale < 1) {
      ...
      const blobOut: Blob | null = await new Promise(res => canvas.toBlob(b => res(b), 'image/jpeg', 0.82));
  ...
  const { error: upErr } = await supabaseClient.storage.from('covers').upload(path, blob, { upsert: true, contentType: 'image/jpeg' });
